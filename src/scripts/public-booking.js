(function () {
  ...
  function renderOptions() {
    ...
    document.getElementById("booking-procedure").innerHTML = state.procedures
      .map(item => `<option value="${item.id}" data-name="${escapeHtml(item.label || item.value)}">${escapeHtml(item.label || item.value)}${Number(item.price || 0) > 0 ? ` - ${formatMoney(item.price, item.priceCurrency || "RSD")}` : ""}</option>`)
      .join("");
  }
